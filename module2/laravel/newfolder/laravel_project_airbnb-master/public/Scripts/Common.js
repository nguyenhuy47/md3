...
function rePosition() {
    ...
    if ($('.ban_scroll .item').length > 0) {
        ...
        var top = window.location.pathname != "/" ? 390 : 486;

        //Chỉ hiển thị từ phần gốc hất lên chi tiết thôi, ko hiển thị khi vuốt quá tin cùng khoảng giá
        if ($(window).scrollTop() > top) {
            //ngta keo qua cai tin 
            $('#SiteLeft .ban_scroll').css({ 'position': 'fixed', 'top': 80, 'left': xleft });
            $('#SiteRight .ban_scroll').css({ 'position': 'fixed', 'top': 80, 'right': xleft });
        } else {
            $('#SiteLeft .ban_scroll').css({ 'position': 'absolute', 'top': top, 'left': xleft });
            $('#SiteRight .ban_scroll').css({ 'position': 'absolute', 'top': top, 'right': xleft });
        }
